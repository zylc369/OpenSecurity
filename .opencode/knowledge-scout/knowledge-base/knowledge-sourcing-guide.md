...
| 信源维护 | staging 变更摘要: 引用提取与计数新发现/末位标记/pending 验证结果，各一行 |
| 遗留 | 发现但本轮未处理的（下轮素材） |
...
**字段二分法**: 机器判定用固定字段——`domain`（主键）/`tier`（S1 站点级·S2 作者级·O 一次性·AG 聚集体入口）/`status`（curated: active·reduced·archive; staging: pending·rejected——rejected 为已审否决留档; 镜像/建议条目可用 curated 侧值 active/reduced/archive 表示建议目标状态）/`last_seen`（最后有效产出日期，YYYY-MM-DD; 数据源只能定位到月时允许 YYYY-MM）/`refs_in`（被引计数）/`added_at`; 指导行为用自然语言——`profile`（是什么/怎么扫/注意什么）、`proposal`（staging 条目的建议）。写入一律 bash（写工具被白名单禁用），读取用 Read 工具或 bash 均可。
...
2. **引用提取与计数**: 读过的 writeup 顺手提取外链（`https?://` 正则），滤噪音后——命中 curated/aggregators 已有源则该源 `refs_in`+1; 新域名追加 staging pending 条目（proposal 写: 引用来源+被引次数）。**计数一律按逻辑引用去重**: 同一文章的双语版本、多平台镜像（zenn+博客双发）、同源转载均算一次——物理出现次数不代表引用强度。滤噪音正则: `gyazo|imgur|st-hatena|zenn\.studio|wikipedia|php\.net|mozilla|chrom(e|ium)|googlesource|whatwg|mitre|x\.com|twitter|tiktok|esm\.sh|02\.rs|transfer\.sh|siam\.org|localhost|127\.0\.0\.1|example|attacker\.com|vulnerable|chal\.ctf|flagyard|forms\.gle|ctfcompetition\.com|\.be\.ax|schema\.org|fonts\.|webhook\.site|oastify|ngrok|requestrepo|t\.co|hexo\.io|unpkg|linkedin|medium\.com|<|server`
...
**引用计数脚本**（对已归档目录跑; scout 读未归档单篇时直接用其中的正则+计数逻辑）:

```bash
python3 << 'PYEOF'
import os, re, glob, collections
DIR = 'docs/资料/writeup-sources/<来源名>'   # ← 改成实际归档目录（或用绝对路径）
# guide 路径探测: $AGENT_DIR 按运行者解析不可靠（evolve 会话指向 evolve 目录），用候选列表取第一个存在的
CANDS = ['.opencode/knowledge-scout/knowledge-base/knowledge-sourcing-guide.md',  # CWD 在仓库根
         '.opencode/../.opencode/knowledge-scout/knowledge-base/knowledge-sourcing-guide.md',  # CWD 在 .opencode
         os.path.join(os.environ.get('AGENT_DIR', '/nonexistent'), 'knowledge-base/knowledge-sourcing-guide.md')]  # scout 子会话（AGENT_DIR=knowledge-scout/）
GUIDE = next((c for c in CANDS if os.path.exists(c)), None) or (_ for _ in ()).throw(FileNotFoundError('guide 未找到: 在仓库根运行或设 AGENT_DIR'))
guide = open(GUIDE, encoding='utf-8').read()
m = re.search(r'滤噪音正则: `([^`]+)`', guide)
if not m: raise SystemExit('错误: 未在 guide §1a 找到滤噪音正则——检查措辞是否被改动')
noise = re.compile(m.group(1))
domains = collections.Counter()
pat = re.compile(r"https?://([^\s\)\]\>\"'`,]+)")
for f in glob.glob(DIR + '/**/*.md', recursive=True):
    for u in pat.findall(open(f, encoding='utf-8', errors='ignore').read()):
        # 去尾部标点、统一小写；保留 host+首路径段（github.com/<user> 形态才能匹配带路径的 curated 键）
        rest = u.rstrip('.').lower().split('/')
        # 仅代码托管域保留首路径段（github.com/<user> 才能对上带路径的 curated 键）; 其他域用裸域避免碎片化
        d = rest[0] + ('/' + rest[1] if rest[0] in ('github.com', 'gitlab.com') and len(rest) > 1 and rest[1] else '')
        if not noise.search(d): domains[d] += 1
for d, c in domains.most_common(): print(c, d)
PYEOF
```

被引 ≥3 次的条目逐个判读: 与 curated/aggregators 的 `domain` 键匹配时用**前缀匹配**（引用计数产出的 `github.com/<user>` 是 curated 键 `github.com/<user>/<repo>` 的前缀，双向任一为前缀即命中）→ `refs_in` 累加; 新条目 → staging pending（proposal 写: 引用来源+被引次数+初判; github 形态的建议在 proposal 里写全 `<user>` 便于后续按 user 扩展）。

staging 写失败（权限/盘满）最多丢本轮增量，主任务不受影响; 文件损坏回退 curated，git 可恢复。

被引 ≥3 次的条目判读时的观察（潜在新源线索、计数异常）写入 staging 对应条目的 proposal——数据归宿在文件; 派发执行时委派方未约束返回格式则默认一行: `引用计数完成: 新 pending N 个 / refs_in 累加 M 处`。
...
4. 不可信 → 条目**保留在 staging**，status 改 `rejected`，否决理由与复活条件写入 proposal——rejected 条目是复活条件的存档，**永不自动清理**
5. 转正完成后 staging 仅清理已转正/pending 已决议条目，rejected 永久保留，`reviewed_at` 更新
