...
│    真实浏览器环境下人机验证多数不触发；用户全程可见，随时可介入
...
提交类操作（flag 提交、表单发送、状态变更）的结果确认，**持久信号优先于瞬态信号**：

```
1. 提交动作前挂 page.on("response") 监听 —— 零成本，响应体是最权威信号
2. 提交后读页面 DOM 文字/状态标志 —— "solved/已完成/通过/correct" 类关键词，
   刷新后仍存在（持久信号），DOM 文本或截图+OCR 均可读取
3. 禁止把 toast 作为确认手段 —— toast 数秒即逝，轮询时机不可控，是最不可靠的信号源;
   不同平台反馈机制不同（toast/行内文字/状态图标/无反馈），先提交一次观察实际机制再写选择器
4. 页面无任何反馈时 → CDP 提取 cookie 后用 HTTP 直调 API，按响应语义判断:
...
```
401 UNAUTHORIZED → 补请求头: Cookie(session) + Origin + Referer + 框架标识头
                （如 tRPC 系加 x-trpc-source: http）
400 参数错误     → 读错误体中的参数 schema 提示改参数——tRPC/zod 系 400 错误体
                  直接列出缺失字段名与 path（如 [{"path":["slug"],"message":"Required"}]），
                  把错误体当 API 文档读，不要盲猜参数名
200/4xx 语义     → 响应体中的状态字段/错误消息即权威结论
```

### 探测端点时的无害验证

用无害请求（错误 flag、空参数）先探测端点有效性和参数 schema，确认后再发真实数据——部分平台对错误提交限次/扣分，避免用真实数据试错烧掉机会。
