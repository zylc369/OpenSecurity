...
1. UPX 壳: `upx -d 目标.bin`（同架构版本不对时换历史版本重试）
...
项目内置 Frida PE 脱壳脚本：`$SHARED_DIR/scripts/frida_unpack.py`。

```bash
$PYTHON_CMD "$SHARED_DIR/scripts/frida_unpack.py" <目标二进制> -o "$TASK_DIR/<文件名>_unpacked" -w 30
```
