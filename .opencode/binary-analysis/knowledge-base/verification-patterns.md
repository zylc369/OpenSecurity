...
**数据提取铁律**: 目标数据（密钥/凭据/长十六进制串）的提取、拼接、校验**必须全程序化**（Python 正则/脚本），绝不手动拼接——LLM 逐字符抄写长串必然出错（少 1-2 字符=截断未察觉、多 1-2 字符=分段重叠、中间错=误读）。程序化提取点: SQLi 用 UNION SELECT 替代报错函数截断（EXTRACTVALUE 32 字符限制）、回显截断改 base64 编码读取、文件包含用 php://filter base64 读后脚本解码、响应中用 `re.search` 带格式正则提取; 提取后过三关校验——长度 assert（已知长度时）/格式正则+结尾符/失败换提取法。分段数据（每段 < 完整长度）必须脚本拼接，禁止跨段手工对齐。
