...
PYTHON_PACKAGES: list[Dependency] = [
    ...
    Dependency(name="PIL", kind="python", pip_name="Pillow", preinstall=True,
               agents=["binary-analysis", "mobile-analysis", "web-analysis"],
               description="Python 图像处理库，用于截图优化（PNG/JPEG 竞争压缩，PSNR≥35dB 自适应）"),
...
EXTERNAL_TOOLS: list[Dependency] = [
    ...
    # ─── 渗透测试工具（web-analysis / mobile-analysis 用）───
    Dependency(
        name="nmap", kind="tool", preinstall=True,
        agents=["web-analysis"], required=False,
        version_cmd=["--version"],
        description="网络端口扫描/服务识别",
        install_hint="nmap 未找到。安装: brew install nmap (macOS) / 参考 https://nmap.org/download.html",
    ),
    Dependency(
        name="sqlmap", kind="tool", preinstall=True,
        agents=["web-analysis"], required=False,
        version_cmd=["--version"],
        description="SQL 注入自动化工具",
        install_hint="sqlmap 未找到。安装: brew install sqlmap (macOS) / 参考 https://sqlmap.org/",
    ),
    Dependency(
        name="gobuster", kind="tool", preinstall=True,
        agents=["web-analysis"], required=False,
        version_cmd=["--version"],
        description="目录/子域名爆破工具",
        install_hint="gobuster 未找到。安装: brew install gobuster (macOS) / 参考 https://github.com/OJ/gobuster",
    ),
    Dependency(
        name="ffuf", kind="tool", preinstall=True,
        agents=["web-analysis"], required=False,
        version_cmd=["-V"],
        description="模糊测试/目录爆破工具",
        install_hint="ffuf 未找到。安装: brew install ffuf (macOS) / 参考 https://github.com/ffuf/ffuf",
    ),
    Dependency(
        name="hydra", kind="tool", preinstall=True,
        agents=["web-analysis"], required=False,
        version_cmd=["-h"],
        description="暴力破解工具",
        install_hint="hydra 未找到。安装: brew install hydra (macOS) / 参考 https://github.com/vanhauser-thc/thc-hydra",
    ),
    Dependency(
        name="nikto", kind="tool", preinstall=True,
        agents=["web-analysis"], required=False,
        description="Web 漏洞扫描器",
        install_hint="nikto 未找到。安装: brew install nikto (macOS) / 参考 https://github.com/sullo/nikto",
    ),
    Dependency(
        name="mitmproxy", kind="tool", preinstall=True,
        agents=["web-analysis", "mobile-analysis"], required=False,
        version_cmd=["--version"],
        description="中间人代理工具",
        install_hint="mitmproxy 未找到。安装: brew install mitmproxy (macOS) / 参考 https://mitmproxy.org/",
    ),
]
