...
| 解密回读的系数与所选明文成乘性关系 + 噪声有界 | 近似模方程 + 区间精炼 | §5.1 |
...
### 5.1 系数乘法泄漏：近似模方程 + 区间精炼恢复秘密

**场景**：服务端把明文系数乘以秘密值 `delta`（或任何乘性秘密）后解密，回读组合系数且噪声有已知上界 `B`——每条泄漏形如 `leak_i = m_i·delta + noise_i (mod q)`，`|noise_i| < B`。典型代码形态：`plain[i + j*coeff_count] = coeff_i * delta mod q_j`（RNS 各 limb 同乘一个 delta）。

**可行性速算（开工前必算）**：每条泄漏携带信息 ≈ `log2(q) - log2(2B)` bit。q≈240 bit、B≈2^188 时每条约 52 bit，几十条方程即可唯一确定 delta；信息量 × 方程数 < 秘密位数则此路不通。

**利用流程**（4 步）：

1. **选合法明文集**：先读服务端明文校验分支（常见：`abs(coeff) < q/8` 抛异常拒绝小系数）。基线取 `base = q/2`（最大合法值），其余槽取 `q/2 + 2^e`（e 在 0..log2(q) 分散取，如隔 2 bit 一个）——既过校验又使各槽系数互异
2. **差分消基线**：`d_e = leak_e - leak_0 (mod q) = 2^e·delta + (noise_e - noise_0)`，大项 `base·delta` 消去，剩同一未知量 delta 的数十条近似模方程，误差 < 2B
3. **区间精炼**：从 `[1, q)` 起，逐方程与 `[d_e - 2B, d_e + 2B]` 在"乘 2^e mod q"下的原像求交。k 的取值直接枚举判定：对每个候选区间 `[L, H]`，遍历 `k = floor(2^e·L / q) - 1 .. floor(2^e·H / q) + 1`（相邻 k 间隔即 q），保留满足 `2^e·L ≤ k·q + d_e + 2B` 且 `k·q + d_e - 2B ≤ 2^e·H` 的 k，反解 `ceil((k·q + d_e - 2B) / 2^e) ≤ delta ≤ floor((k·q + d_e + 2B) / 2^e)`。全部方程过完候选缩到个位数
4. **共享噪声过滤定唯一**：所有 d_e 含**同一个** `noise_0`。对每个候选 delta 算中心化残差 `r_e = center(d_e - 2^e·delta mod q)`；要求存在单一 `noise_0` 满足 `-B ≤ r_e + noise_0 ≤ B` 对全部 e 成立，即区间族 `[-B - r_e, B - r_e]` 的交集非空。真值非空、相邻错值空

**检查**：泄漏是否线性于所选明文（加倍明文系数看泄漏差是否加倍）；噪声界是否在代码中显式（如 `cpp_int(5) << 185`）或可由重复查询估计；明文系数校验的具体阈值。
