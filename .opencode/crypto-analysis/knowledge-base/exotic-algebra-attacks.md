...
编码基于小群（阶 ≤ 几千，如二十面体群 120）+ 加密 oracle——单位探测（0..k-1 单值加密）建全查找表即解。部分可见（6/12 面等）先对碰撞率做实际测量。
...
## 14a. 矩阵多项式 mod 隐藏素数（交换关系 + Sylvester 链 + CVP）

**适用**: 服务端把小整数矩阵块 A,B,C 堆成 48×48 上三角块阵 `x = [[A,B,C],[0,A,B],[0,0,A]]`，只公布 `cap = f(x) mod p`（p 隐藏，如 2048-bit; f 是已知多项式 `x^4097 + 3x^257 + 11x^17 + 42x + 99`）。

**恢复链**（对角块 Y = f(A) 已知，A 未知）:

1. **交换关系消模商**: `A·Y ≡ Y·A (mod p)` 逐元素给出 `t[i,j]·p = ΣA·Y - ΣY·A`——**共享行/列的两个方程交叉相乘消去 p**，得到含 `t·A` 的短整数解——48 维格 `[I | u]` LLL 后按 `mix()` 系数 `1+d[i]-d[j]` 整除（商须为字节）筛选符号与尺度，逐列恢复非对角 A
2. **模数恢复（标量歧义）**: 交换关系对**标量矩阵不敏感**（A+cI 同样满足）——设对角差 δ = A[i,i]-A[j,j] ∈ [-255,255]，对两对 (i,j) 枚举 δ 算 `p | Σ(δ·Y[i,j])` 取 **GCD** 得 2048-bit 素数 p（第三对去小余因子 + 素性检查）
3. **对角线恢复**: 模 p 除法解出全部 δ 后，公共移位 c 用**代入验证**: `f(A_candidate) == Y`（模 p 矩阵幂）从少量候选选中
4. **上三角块递推（Sylvester 线性化）**: 非对角块满足 `Y·B - B·Y = A·Z - Z·A`（及含 `BZ-ZB` 的三块版）——每块是 240 个非对角未知数的**仿射线性方程组**（16 个对角当参数），解仿射系后对角部分: "每元素除以 mix 系数是字节"的界 → **24 样本 LWE/CVP**——40 维 embedding LLL + Babai 最近平面恢复
5. **移位与终验**: 剩余公共标量用 **Fréchet 导数 f'(A)** 与已知块（Z 或 W）消去; 每块恢复后对完整 48×48 公开块验算

**识别信号**: "matrix"/"48×48"/公布值是"多项式作用后 mod 未知 p"/块状重复对角。**关键认知**: 隐藏模数不改变 `M·f(M) = f(M)·M` 的线性结构——格攻击直接作用于"消去模商后的整数方程"，模数本身反而可由对角差 GCD 免费恢复。

**工具**: python-flint `fmpz_mat.lll()`; Babai 最近平面自实现（~20 行）。
