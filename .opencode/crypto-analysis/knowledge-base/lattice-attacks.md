...
## 5i. 稀疏多项式 oracle（少查询恢复 t 项多项式）

**场景**: 服务只允许极少次（如 8 次）求值 `f(x_1..x_n) = Σ c_j·m_j(x) mod p`，f 是 n 变量、总次数 ≤D 的 t 项稀疏多项式（t ≪ C(n+D,D)），求 p 与系数。

1. **小坐标探测**: 每个查询点坐标全取自 {1,2,3} → 单项式取值 `M[i,j]=m_j(probe_i) ≤ 3^D` 有界; 输出 `y = M·c (mod p)`
2. **LLL 找输出的短整关系**: M 是 8×5 时左零空间维 3，模 p 下 `w·y ≡ 0`; 对 y 的整数零空间格 LLL 归约——真实多项式对应的关系比随机关系显著短，取最短两条
3. **弱组合枚举过滤**: 枚举全部度 ≤D 候选单项式的**评估签名列**（如 705,432 列），保留同时满足两条短关系的列 → 恰好剩下真实 t 列（小坐标保证签名互异）
4. **模数恢复**: [M|y] 的 8×6 矩阵 mod p 秩 ≤5 → 所有 6×6 子行列式的 GCD 含 p（混入探测点引入的 2/3 幂小因子，除掉即 p）; 再在 GF(p) 解 M·c=y 得系数

判定信号: "oracle 限极少次 + 多变量 + 提到 sparse/项数小" 即走本节; LLL 短向量与次短有明显 gap 是关系正确的标志。
