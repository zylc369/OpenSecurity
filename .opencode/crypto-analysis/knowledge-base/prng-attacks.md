...
- **SplitMix64**: 指纹常数 `0x9E3779B97F4A7C15`（黄金比例）；输出混合 `z = ((x^(x>>30))*C1); z = ((z^(z>>27))*C2); z ^ (z>>31)`（C1=0xBF58476D1CE4E5B9, C2=0x94D049BB133111EB）。两个可利用特性：① 状态推进只是 `state = (state + GOLDEN) mod 2^64`——**跳过 skip 步有闭式 `(state + skip*GOLDEN) mod 2^64`**，枚举种子时免迭代 ② 输出混合函数双射，完整输出可反解 state。典型形态：时间戳/小窗口种子驱动 RSA 素数生成——枚举窗口内全部种子，用 `abs(n - p_base*q_base)` 快速过滤（p = next_prime(p_base)，偏差 ≈ p_base·δq + q_base·δp ≈ 2^(bitlen/2) × 素数间隙上界；384-bit 素数平均间隙 ~2^8，阈值取 `2^(bitlen(p_base)+50)` 量级留足余量——随机种子的乘积差是全幅随机量，命中即真），再 `next_prime(p_base)*next_prime(q_base) == n` 终验
...
## 8a. 未知模数 + 截断高阶线性递推（零化多项式 + 结式）

**适用**: `A[i+n] = Σ C[j]·A[i+j] mod p`（高阶递推，n=25 级），模数 p 未给出但**接近 2^k**（如 `p = 2^128 - d, d < 2^50`），只泄漏每态高 80 位（`Y_i = A_i >> 48`）。

**三步恢复**:

1. **零化多项式（模数未知也能找）**: 长度 ~3n 的零化向量 η 满足 `Σ η_i·Y_{i+j}` 很小——因 `2^128 - p` 小，截断后的线性关系仍是格里的短向量。**坐标缩放**: `2^128-p` 的上界是隐藏范围的 m 倍时，系数坐标乘 m 平衡残差坐标。180 维 BKZ-35 暴露多个独立的 degree-(n-1) 零化多项式。
2. **结式恢复模数**: 两个独立零化多项式的**结式（resultant）含 p^n 因子**——多组结式两两 GCD 即恢复 p（`gcd(Res(f,g), Res(f,h))`）。
3. **taps 与状态**: `F_p[x]` 中归约并 GCD 两多项式得**特征多项式**（degree=n）→ 取负系数即递推系数 C。状态恢复转 BDD/CVP: `A_i = 2^48·Y_i + Z_i`，居中 `Z_i = W_i + 2^47` 后 `q_j·W - t_j = W_j (mod p)` 全未知量 < 2^47——构造 `[I | Q; 0 | pI]` 格 LLL + Babai 最近平面恢复低 48 位; 用**全部 180 个观测重生成比对**排除格假阳性。`C[0]^-1 mod p` 逐步倒推 n 次恢复初始状态（key 派生自完整序列时必需）。

**识别信号**: 输出"看起来是高位的定长数"、递推阶数已知、模数缺失、`2^k - p` 小（题目给 p 生成区间）。

**工具**: python-flint `fmpz_mat.lll()`/`fmpz_mod_poly`（结式与 GCD）。
