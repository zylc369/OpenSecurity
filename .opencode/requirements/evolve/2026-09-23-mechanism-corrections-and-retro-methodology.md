...
| E | 记忆库（knowledge.db，经控制台 API） | 追加 2 条更正条目（C-A/C-B） | ±60 |
...
- **方法（选定）**：追加 2 条更正条目（C-A / C-B），正文含正确机制 + "被取代条目清单"；检索到旧条目时以更正条目为准。
- **否决的方案**：
...
- 文件：D2 | 预估：+1 行 | 依赖：S7
- 要点：增量控制在索引表 1 行（该 prompt 展开行数已 >600，Phase 4.5 要求先瘦身——本次以最小索引行落地，完整瘦身按既有待办另行排队，不在本需求范围）
