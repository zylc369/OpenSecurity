# [开源]OpenSecurity：AI 驱动的多领域安全自动分析 Agent 平台
...
- **真的会操作工具**：不是给建议，是自己启动 `idat` 跑 IDAPython 脚本、读 JSON 输出、做下一步推理
- **长会话不迷路**：上下文压缩后自动恢复关键状态（TASK_DIR、已识别的函数、已尝试的方向），分析能跑几个小时不中断
- **越用越强**：每次实战经验被蒸馏成可复用的脚本和知识库，下次遇到类似场景直接调用
- **多领域协同**：复合任务自动拆分（逆向 APK → 分析 native 加密 → 验证后端接口），分发给对应领域的专业 Agent，结果聚合到一份报告
...
超过最大持续时间（Plugin 内置默认 6 小时）后不再自动恢复，避免无限循环。
